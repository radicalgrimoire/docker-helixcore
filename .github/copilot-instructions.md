...
GitHub のリポジトリルールセットにより、`main` ブランチへの直接コミットおよび push はできません。
変更を行う際は必ず作業用の別ブランチを作成し、そのブランチ上でコミットしてください。
...
### GitHub Actions
- **test.yml**: 汎用化されたテストワークフロー（`workflow_call` 使用）
- **publish.yml**: 汎用化されたパブリッシュワークフロー（`workflow_call` 使用）
- **再利用性**: 入力パラメータによる動的な動作制御
- **アーティファクト管理**: Docker イメージの効率的な受け渡し
...
- **test.yml**: 再利用可能なテストワークフロー
  - `workflow_call` による他のワークフローからの呼び出し対応
  - `docker_files_name` パラメータによる柔軟なDockerfile指定
  - `should_push` パラメータによる条件付きアーティファクト保存
  - Docker イメージのテスト実行とアーティファクト管理
- **publish.yml**: 再利用可能なパブリッシュワークフロー
  - `version` と `version_raw` パラメータによる動的タグ生成
  - 複数タグ戦略（バージョン+ビルド番号、latest、nightly）
  - GitHub Container Registry への自動プッシュ
  - スケジュール実行時の特別なタグ処理
